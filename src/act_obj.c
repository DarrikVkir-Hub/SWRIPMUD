...
void get_obj( CHAR_DATA *ch, OBJ_DATA *obj, OBJ_DATA *container )
{
    ...
    if ( !CAN_WEAR(obj, ITEM_TAKE)
       && (ch->top_level < ch->game->get_sysdata()->level_getobjnotake )  )
    {
...
void do_get( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->in_room && BV_IS_SET(ch->in_room->room_flags, ROOM_PLR_HOME) && get_trust(ch) < LEVEL_SUB_IMPLEM )
    {
	...
	if ( number <= 1 && str_cmp( arg1, "all" ) && str_prefix( "all.", arg1 ) )
	{
	    ...
	    if ( char_died(ch) )
		return;
	    if ( BV_IS_SET( ch->game->get_sysdata()->save_flags, SV_GET ) )
	    {
		save_char_obj( ch );
	    ...
	    for ( obj = container->first_content; obj; obj = obj_next )
	    {
			...
			&&   can_see_obj( ch, obj ) )
			{
					...
					||   (number && cnt >= number) )
				{
					...
					if( found && BV_IS_SET( ch->game->get_sysdata()->save_flags, SV_GET ) )
						save_char_obj( ch );
					return;
				}
			}
...
void do_put( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !container->carried_by && BV_IS_SET( ch->game->get_sysdata()->save_flags, SV_PUT ) )
	save_char = TRUE;
...
void do_give( CHAR_DATA *ch, char *argument )
{
	...
	if ( BV_IS_SET( ch->game->get_sysdata()->save_flags, SV_GIVE ) && !char_died(ch) )
	  save_char_obj(ch);
	if ( BV_IS_SET( ch->game->get_sysdata()->save_flags, SV_RECEIVE ) && !char_died(victim) )
...
obj_ret damage_obj( OBJ_DATA *obj )
{
    ...
    if (ch && ch->in_room && BV_IS_SET(ch->in_room->room_flags,ROOM_ARENA))
      return objcode;
...
void do_auction (CHAR_DATA *ch, char *argument)
{
    ...
    if ( !BV_IS_SET( ch->in_room->room_flags , ROOM_HOTEL ) && !BV_IS_SET( ch->in_room->room_flags , ROOM_SAFE ) )
    {
...
/*          spell_identify (0, LEVEL_HERO - 1, ch, auction->item); */
	...
	if ( BV_IS_SET( ch->game->get_sysdata()->save_flags, SV_AUCTION ) )
...
//  char buf[MAX_STRING_LENGTH];
    ...
    if (fall_count > 30)
    {
     ...
     if ( BV_IS_SET( obj->in_room->room_flags, ROOM_NOFLOOR )
     &&   CAN_GO( obj, DIR_DOWN )
	...
	if (obj->in_room->first_person)
	{
 	...
 	if (!BV_IS_SET( obj->in_room->room_flags, ROOM_NOFLOOR ) && through )
	{
